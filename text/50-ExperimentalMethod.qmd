...
**Magnitude of effects.** We control the magnitude of effects by varying $a_1$, $a_2$, and, for some experiments (those evaluating power for interactions), $a_{12}$. @fig-effects-normal presents normal distributions ($\sigma^2 = 1$) generated across the range of values of $a_1$ used to evaluate Type I error rates. In these examples, we illustrate effects for a factor with three categorical levels and, for clarity, disregard between-subject variability ($\sigma_s^2 = 0$).
...
plot_effects_distr()
```
Different ratio distributions generated with $a_1 = 2$ when $X_1$ has three categorical levels (here, $\sigma_s^2 = 0$).
:::

In practice, we randomly choose the standard deviation $\sigma_s^2$ --- which determines between-subject variability --- from the interval $[0.1, 0.5]$, drawing each value from a uniform distribution.
