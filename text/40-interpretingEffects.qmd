...
### Controversy about the interpretation of interactions
...
We emphasize that the existing literature does not clarify how ART is expected to interpret main and interaction effects in these scenarios. Early evaluations of ART did not consider nonlinear models, so the question was not even posed. @elkin:2021 who do study such models, use log-transformed values as their baseline for log-normal distributions (see their example on Page 4). So it seems that Elkin et al. assume ART's conclusions should align with the coefficients of the linear submodel. Unfortunately, their evaluations focus on contrasts and do not account for parallel effects, avoiding interpretation issues.
