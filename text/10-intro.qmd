...
However, given their substantial impact on research outcomes in our community, these widely adopted methods deserve closer scrutiny. As the first author previously argued [@tsandilas:2018], the HCI community often lacks the necessary expertise to critically assess the validity of statistical techniques. Our findings suggest that the widespread use of ART largely stems from a lack of awareness regarding its underlying assumptions. Many researchers treat ART as a general-purpose nonparametric alternative when ANOVA's assumptions are violated, overlooking the fact that ART’s alignment mechanism imposes even stricter assumptions -- including a linear relationship between the response and predictors, equal variances, and continuous data.
