...
However, given the significant influence of such milestone methods on our community's research outcomes, they warrant a closer scrutiny. Our results suggest that ART's popularity step largely from an ignorance of the method's assumptions. Researchers often treat ART as a general-purpose nonparametric alternative when ANOVA's assumptions are violated, overlooking the fact that ART's alignment mechanism introduces stricter assumptions than ANOVA itself, including a linear relationship between the response variable and the effects, equal variances, and continuous responses. 
