...
distributions <- c("lognormal", "exponential", "cauchy", "poisson")
...
handlers(handler_txtprogressbar)   # To enable progress output

with_progress({
  p <- progressor(steps = length(distributions) * length(Ns) * nrow(effects))
