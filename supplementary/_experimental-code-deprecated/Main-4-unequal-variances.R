...
# Experiment evaluating the Type I error rates of PAR, ART, RNK, and INT under unequal variances.
# See Fig. 22, Fig. 23, Fig. 24
...
test.between.2f <- function(n, design, effects, distr, sd_ratio = 1){
	df <- sim.between.2f.hetero(design[1]*design[2]*n, design, effects, sd_ratio = sd_ratio) %>% toDistribution(distr) # Ensure n participants per cell
...
test.mixed.2f <- function(n, design, effects, distr, sd_ratio = 1){
	df <- sim.mixed.2f.hetero(design[1]*n, design, effects, sd_ratio = sd_ratio) %>% toDistribution(distr) # Ensure n participants per cell
...
test.within.2f <- function(n, design, effects, distr, sd_ratio = 1){ 
	df <- sim.within.2f.hetero(n, design, effects, sd_ratio = sd_ratio) %>% toDistribution(distr)
...
test <- function(repetitions, n, designId, design, effects, distr, sd_ratio) {
	results <- foreach(rid = 1:repetitions, .combine=rbind) %dopar% {
		tryCatch(
			{
				testFun(designId)(n, design, effects, distr, sd_ratio)
			}, 
			error = function(cond) {
				# do nothing
			}, finally = {
	...
	return(tribble(~n, ~design, ~distr, ~sd_ratio, ~method, ~alpha, ~effects, ~rates,
			n, designStr, distr, sd_ratio, "PAR", 0.05, effects, res.05[1:3],		
			n, designStr, distr, sd_ratio, "RNK", 0.05, effects, res.05[4:6],
			n, designStr, distr, sd_ratio, "ART", 0.05, effects, res.05[7:9],
			n, designStr, distr, sd_ratio, "INT", 0.05, effects, res.05[10:12],

			n, designStr, distr, sd_ratio, "PAR", 0.01, effects, res.01[1:3],		
			n, designStr, distr, sd_ratio, "RNK", 0.01, effects, res.01[4:6],
			n, designStr, distr, sd_ratio, "ART", 0.01, effects, res.01[7:9],
			n, designStr, distr, sd_ratio, "INT", 0.01, effects, res.01[10:12]
		)
	)
}
...
effects <- c(0, 0, 0) # Test Type I error rates, so the effect size is set to zero
sd_ratios <- c(1, 1.5, 2, 2.5, 3)  # max SD ratios (unequal variances)

# B. 2 x 3 between-subjects, 2 x 4 mixed-subjects, and 4x3 within-subjects designs
designs <- list(c(2,3), c(2,4), c(4,3))

n <- 20
R <- 5000

filename = "4_test_Unequal_Variances"
	...
	results <- foreach(desId = 1:length(designs), .combine=rbind) %do% {
		foreach(distr = distributions, .combine=rbind) %do% {
			foreach(ratio = sd_ratios, .combine=rbind) %do% {
				...
				test(repetitions = R, n = n, designId = desId, design = designs[[desId]], effects = effects, distr = distr, ratio)
			}
		}
	}
})
