...
# Experiment evaluating the Type I error rates of PAR, ART, RNK, and INT for main effects in the presence of interactions
# See Appendix: Fig. 4, Fig. 5, Fig. 6
...
categ2ratio_eq <- function(nlevels) { 
...
test.between.2f <- function(n, design, effects, distr){
	df <- sim.between.2f(design[1]*design[2]*n, design, effects, convertFun = categ2ratio_eq) %>% toDistribution(distr) # Ensure n participants per cell
...
test.mixed.2f <- function(n, design, effects, distr){
	df <- sim.mixed.2f(design[1]*n, design, effects, convertFun = categ2ratio_eq) %>% toDistribution(distr) # Ensure n participants per cell
...
test.within.2f <- function(n, design, effects, distr){ 
	df <- sim.within.2f(n, design, effects, convertFun = categ2ratio_eq) %>% toDistribution(distr)
...
filename = "Appendix_test_growing-interactions"
