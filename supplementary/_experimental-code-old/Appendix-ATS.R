...
# Experiment evaluating the Type I error rates of PAR, RNK, INT, ATS (ANOVA-type statistic) 
# See Appendix: Fig. 26, Fig. 27, Fig. 28, Fig. 29 
...
#	model2 <- suppressMessages(art(y ~ x1*x2 + (1|s), data=df)) # ARTool
	...
	model2 <- suppressMessages(nparLD(y ~ x1*x2, data=as.data.frame(df), subject="s", order.warning=F, description=FALSE))
	model3 <- suppressMessages(lmer(inverseNormalTransform(y) ~ x1*x2 + (1|s), data=df)) # INT
	vars <- c("x1", "x2", "x1:x2")
	c(getLmerPValues(model0, vars), getLmerPValues(model1, vars), getATSPValues(model2), getLmerPValues(model3, vars))
}
...
# Choose the data generation function for the two designs 
testFun <- function(id) {switch(id, test.mixed.2f, test.within.2f)} 
...
test <- function(repetitions, n, designId, design, effects, distr) {
	...
	return(tribble(~n, ~design, ~distr, ~method, ~alpha, ~effects, ~rates,
			...
			n, designStr, distr, "ATS", 0.05, effects, res.05[7:9],
			...
			n, designStr, distr, "ATS", 0.01, effects, res.01[7:9],
...
# B. 2 x 4 mixed design, and 4 x 3 repeated measures
designs <- list(c(2,4), c(4,3))
