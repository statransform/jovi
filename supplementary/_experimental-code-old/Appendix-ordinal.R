...
# Experiment evaluating the Type I error rates of PAR, ART, RNK, and INT for ordinal data under a wider range of designs:
# 2x3 between-subjects, 2x4 mixed-subjects, 2x2x2 within-subjects, and 3x3x3 within-subjects
# See Appendix: Fig.17, Fig.18, Fig.19
...
test.within.3f <- function(n, design, effects, distr){
	...
	model0 <- suppressMessages(lmer(y ~ x1*x2*x3 + (1|s), data=df)) # Parametric
	model1 <- suppressMessages(lmer(rank(y) ~ x1*x2*x3 + (1|s), data=df)) # RNK
	model2 <- suppressMessages(art(y ~ x1*x2*x3 + (1|s), data=df)) # ARTool
	model3 <- suppressMessages(lmer(inverseNormalTransform(y) ~ x1*x2*x3 + (1|s), data=df)) # INT
...
# Specify the generation functions used for each design
testFun <- function(id) {switch(id,test.between.2f, test.mixed.2f, test.within.3f, test.within.3f)} 
...
# B. (1) 2 x 3 between-subjects, (2) 2 x 4 mixed design (one between-subjects and one repeated-measures factor), (3) 2 x 2 x 2 repeated measures, and (4) 3 x 3 x 3 repeated measures
designs <- list(c(2,3), c(2,4), c(2,2,2), c(3,3,3))

# C. Various ordinal scales
...
filename = "Appendix_test_ordinal"
