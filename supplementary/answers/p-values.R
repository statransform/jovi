...
test.within.2f <- function(n, design, effects, distr){ 
	...
	vars <- c("x1", "x2", "x1:x2")
	c(effects, 
		getLmerPValues(model, vars), getLmerPValues(model0, vars), getLmerPValues(model1, vars), getARTPValues(model2, vars), getLmerPValues(model3, vars),
		eta_values(model), eta_values(model0), eta_values(model1), eta_values(model2), eta_values(model3))
}
...
time <- system.time({ 
	results <- 
		foreach(n = Ns, .combine=rbind) %do% {
			...
			test(repetitions = R, n = n, design = design, distr = distr, effects = effects)
		}
})
...
colnames(res)[22:24]=c("eta_X1","eta_X2","eta_X1X2")
colnames(res)[25:27]=c("eta_parX1","eta_parX2","eta_parX1X2")
colnames(res)[28:30]=c("eta_rnkX1","eta_rnkX2","eta_rnkX1X2")
colnames(res)[31:33]=c("eta_artX1","eta_artX2","eta_artX1X2")
colnames(res)[34:36]=c("eta_intX1","eta_intX2","eta_intX1X2")
