...
vars <- c("x1", "x2", "x1:x2")
res <- c(getLmerPValues(model0, vars), getARTPValues(model2, vars), getLmerPValues(model1, vars), getLmerPValues(model3, vars), getLmerPValues(model4, vars))
...
#print(round(res, digits = 20))
# Store the results
#csvfile <- paste("logs/", filename, format(Sys.time(), "_%s"), ".csv", sep="")
write.csv(df, file = "logs/data.csv", row.names=FALSE, quote=F)
