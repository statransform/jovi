---
title: "Rebuttal - Complimentary example (homoscedastic log-normal data)"
...
We detail the analysis for a complimentary example where data were drawn from homoscedastic log-normal populations. We present this example in our [first answer](https://github.com/journalovi/2024-tsandilas-ranktransforms/issues/2#issuecomment-2483020115) to the commentary of Prof. James Higgins.

As in our original illustrative example, we use [data](example-data-2.csv) from a hypothetical experiment that evaluates the time performance of three user interface techniques (*A*, *B*, and *C*) that help users complete image editing tasks of four different difficulty levels: *Level1*, *Level2*, *Level3*, and *Level4*. 
...
model1 <- glmer(Time ~ Difficulty*Technique + (1|Participant), family = gaussian(link = "log"), data = df)
model2 <- glmer(log(Time) ~ Difficulty*Technique + (1|Participant), family = gaussian(link = "identity"), data = df)
