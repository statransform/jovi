...
# dnames = c("Normal", "Log-normal", "Exponential", "Poisson", "Binomial", "Ordinal (5 levels)", "Ordinal (7 levels)", "Ordinal (20 levels)")
 ...
 #df <- df %>% arrange(design,family,effectX1X2,etaX1X2)  %>% group_by(design,family,effectX1X2) #%>% mutate(rank = rank(etaX2))
 #df <- as.data.frame(df) %>% reshapeByDesign(dnames, effectvars = c("effectX1","effectX2","effectX1X2"),  groupvars = c("family","method","n"))
